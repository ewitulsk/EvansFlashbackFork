...
public class ReplayUI {
    ...
    private static final CustomImGuiImplSdl imguiSdl = new CustomImGuiImplSdl();
    public static final CustomImGuiImplB3D imguiRenderer = new CustomImGuiImplB3D();

    public static CustomImGuiImplSdl imguiSdl() {
        return imguiSdl;
    }
    ...
    public static void init() {
        ...
        imguiSdl().init(Minecraft.getInstance().getWindow().handle());
        imguiRenderer.init();
    ...
    public static void initFonts(String languageCode) {
        ...
        for (int scancode = 0; scancode < SDLScancode.SDL_SCANCODE_COUNT; scancode++) {
    ...
    public static ImGuiContext getImGuiContext() {
        if (!initialized) {
            init();
        }
        return imGuiContext;
    ...
    private static void handleBasicInputs() {
        ...
        if (ImGui.isMouseClicked(ImGuiMouseButton.Left)) {
            int key = -SDLMouse.SDL_BUTTON_LEFT-1;
            if (key != 0) {
